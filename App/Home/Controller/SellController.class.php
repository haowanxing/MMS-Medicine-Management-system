...
class SellController extends Controller{
    ...
    public function getSellList(){
        ...
        $result = $this->db->join(
            "LEFT JOIN __ORDER__ ON __ORDER__.orderno = __SELL__.orderno"
        )->join("__STOCK__ ON __STOCK__.stock_id = __SELL__.stock_id")->join(
            "__DRUGS__ ON __DRUGS__.drug_id = __STOCK__.drug_id"
        )->join("LEFT JOIN __USERS__ ON __USERS__.id = __ORDER__.sell_by")
            ->page($page, $limit)->order("sell_id DESC")->select();
        ...
        foreach ($result as $key => $value){
            ...
            $string .= "<td>".($value['sell_status'] == 0 ? "正常" : "退货")
                ."</td>";
            $string .= "</tr>";
        }
    ...
    public function setDataFromPost(){
        $this->setStockId(I("post.stock_id"))->setPrice(I("post.price"))
            ->setSellAmount(I("post.amount"))->setAllprice(
            $this->price * $this->sell_amount
        )->setTime(time())->setSellBy(I("session.userId"));
        return $this;
    }
    ...
    public function getInfo(){
        if (!empty(I("post.sell_id"))){
            $this->data = $this->setSellId(I("post.sell_id"))->formatData(
            )->db->where($this->data)->join("LEFT JOIN __ORDER__ ON __ORDER__.orderno = __SELL__.orderno")->join(
                "__STOCK__ ON __STOCK__.stock_id = __SELL__.stock_id"
            )->join("__DRUGS__ ON __DRUGS__.drug_id = __STOCK__.drug_id")->join(
                "__USERS__ ON __USERS__.id = __ORDER__.sell_by"
            )->find();
            if ($this->data){
                $retMsg = array("code"   => 200, "msg" => "ok",
                                "result" => $this->data);
            }else{
                $retMsg = array("code"   => 400,
                                "msg"    => "编号".I("post.sell_id")."不存在",
                                "result" => $this->data);
            }
        }else{
